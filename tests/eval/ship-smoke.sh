...
PY
  ...
  fi
  {
    ...
    printf '## Task add-doc\n\n'
    printf 'Write a short `docs/NOTE.md` describing the division-by-zero behavior\nrequired by tests/test_calc.py (`divide(x, 0)` returns `None`), without\ndescribing `src/calc.py`'"'"'s implementation.\n'
  } > "$PLAN"
  git -C "$REPO" init -q
