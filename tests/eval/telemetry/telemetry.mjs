...
// --sessions (see codex.mjs), splits the orchestrator's own wall-clock time
// into model / tool:<name> / waiting / user buckets, and reports per-child
// role, model, tokens and cost. Never calls a model; pure log analysis.
...
const USAGE = `usage: node telemetry.mjs codex --root <rollout.jsonl> [--sessions <dir>] [--from ISO] [--to ISO] [--prices prices.json] [--json]`
...
export function parseArgs(argv) {
  if (argv[0] !== 'codex') {
    throw new Error(`unknown or missing subcommand${argv[0] ? `: ${argv[0]}` : ''}\n${USAGE}`)
  }
  const opts = { json: false }
  const flags = { '--root': 'root', '--sessions': 'sessions', '--from': 'from', '--to': 'to', '--prices': 'prices' }
...
function main(argv) {
  ...
  const report = buildReport(run, { from: opts.from, to: opts.to, prices })
  if (opts.json) {
