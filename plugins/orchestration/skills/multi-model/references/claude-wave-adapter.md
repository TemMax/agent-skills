...
4. Act on the returned statuses, task by task:
   ...
   - `environment-blocked` — the machine, not the work, stopped the task: name
     the command that hit it and the verbatim error line, read from the
     task result's `environment` object (`{source, line}`, plus `cmd` when a
     `must_run` command was the one that failed), fix that on the machine,
     then re-run the wave. The re-run is a fresh `Workflow({ scriptPath })`
     call from a launch script regenerated by `wave-launch.mjs` (step 2
     above), never `resumeFromRunId` on the same run — `resumeFromRunId`
