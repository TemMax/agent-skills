...
// A launch script generated by wave-launch.mjs embeds the wave input as
// WAVE_ARGS right after meta; a direct invocation passes it as args.
let wave = typeof WAVE_ARGS !== 'undefined' ? WAVE_ARGS : args
...
function verifierPrompt(t, report) {
  ...
  return [
    ...
    '   that whole command line ($? after it), never of a command inside it. A',
    '   leading ! negates, | pipes and &&/|| chain: they are part of the',
    '   command, so never strip, split or reinterpret them. Record the output',
    '   too (tail is fine for long green output; keep failures verbatim).',
