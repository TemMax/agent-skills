#!/usr/bin/env bash
#
# announce-changelog.sh — announce every newly released CHANGELOG.md section on Telegram.
#
...
# commits (via changelog-new-versions.sh) and announces each one's section, reading it out of
# `git show <after>:CHANGELOG.md`. In the --version form, announces that section of the
# working tree's CHANGELOG.md directly — used to (re)announce a release by hand. --dry-run is
# passed through to telegram-notify.sh.
#
# Prints "no new CHANGELOG.md release section" and exits 0 when there is nothing to announce.
# Tries every version even if an earlier one failed, and exits 1 if any announcement failed.
...
announce_section() {
    ...
    notes="$(mktemp)"
    tmpfiles+=("$notes")
    if ! "$SCRIPT_DIR/changelog-section.sh" "$version" "$file" > "$notes"; then
        return 1
    fi
